...
CMD3Mesh::CMD3Mesh()
{
	m_md3File.md3Mesh=NULL;
	m_md3File.md3Frame=NULL;
	m_md3File.md3Tag=NULL;
	ZeroMemory(&m_md3File.md3Header, sizeof(MD3HEADER));
...
{
	if(m_bMD3Loaded){
		*lpNumTags=m_md3File.md3Header.lNumTags;
		return S_OK;
	}else{
		*lpNumTags=0;
		return E_FAIL;
...
{
	if(m_bMD3Loaded && (lRef>0) && (lRef<=m_md3File.md3Header.lNumTags)){
		strcpy(szTagName, m_md3File.md3Tag[lRef-1].szName);
		return S_OK;
	}else{
		strcpy(szTagName, "");
		return E_FAIL;
	...
	D3DMATRIX * Translation)
{
	...
	lTagRefFirst=dwFirstFrame*m_md3File.md3Header.lNumTags+dwTagRef-1;
	lTagRefSecond=dwSecondFrame*m_md3File.md3Header.lNumTags+dwTagRef-1;

	D3DXMatrixIdentity((D3DXMATRIX*)Translation);

	if( (dwTagRef<1) || (((LONG)dwTagRef) > m_md3File.md3Header.lNumTags))
		return E_FAIL;

	if( (dwFirstFrame < 0) ||
		(dwSecondFrame < 0) ||
		(dwFirstFrame >= m_md3File.md3Header.lNumFrames) ||
		(dwSecondFrame >= m_md3File.md3Header.lNumFrames))
		return E_FAIL;
	
	FirstFrame._11=m_md3File.md3Tag[lTagRefFirst].vAxis[0].x;
	FirstFrame._12=m_md3File.md3Tag[lTagRefFirst].vAxis[0].y;
	FirstFrame._13=m_md3File.md3Tag[lTagRefFirst].vAxis[0].z;
	FirstFrame._14=0;

	FirstFrame._21=m_md3File.md3Tag[lTagRefFirst].vAxis[1].x;
	FirstFrame._22=m_md3File.md3Tag[lTagRefFirst].vAxis[1].y;
	FirstFrame._23=m_md3File.md3Tag[lTagRefFirst].vAxis[1].z;
	FirstFrame._24=0;

	FirstFrame._31=m_md3File.md3Tag[lTagRefFirst].vAxis[2].x;
	FirstFrame._32=m_md3File.md3Tag[lTagRefFirst].vAxis[2].y;
	FirstFrame._33=m_md3File.md3Tag[lTagRefFirst].vAxis[2].z;
	FirstFrame._34=0;
	...
	SecondFrame._11=m_md3File.md3Tag[lTagRefSecond].vAxis[0].x;
	SecondFrame._12=m_md3File.md3Tag[lTagRefSecond].vAxis[0].y;
	SecondFrame._13=m_md3File.md3Tag[lTagRefSecond].vAxis[0].z;
	SecondFrame._14=0;

	SecondFrame._21=m_md3File.md3Tag[lTagRefSecond].vAxis[1].x;
	SecondFrame._22=m_md3File.md3Tag[lTagRefSecond].vAxis[1].y;
	SecondFrame._23=m_md3File.md3Tag[lTagRefSecond].vAxis[1].z;
	SecondFrame._24=0;

	SecondFrame._31=m_md3File.md3Tag[lTagRefSecond].vAxis[2].x;
	SecondFrame._32=m_md3File.md3Tag[lTagRefSecond].vAxis[2].y;
	SecondFrame._33=m_md3File.md3Tag[lTagRefSecond].vAxis[2].z;
	SecondFrame._34=0;
	...
	x = m_md3File.md3Tag[lTagRefFirst].vPosition.x + 
			fTime * 
			(	m_md3File.md3Tag[lTagRefSecond].vPosition.x - 
				m_md3File.md3Tag[lTagRefFirst].vPosition.x);

	y = m_md3File.md3Tag[lTagRefFirst].vPosition.y + 
			fTime * 
			(	m_md3File.md3Tag[lTagRefSecond].vPosition.y - 
				m_md3File.md3Tag[lTagRefFirst].vPosition.y);

	z = m_md3File.md3Tag[lTagRefFirst].vPosition.z + 
			fTime * 
			(	m_md3File.md3Tag[lTagRefSecond].vPosition.z - 
				m_md3File.md3Tag[lTagRefFirst].vPosition.z);
	...
	DWORD dwFlags)
{
	...
	if( (lMesh < 1) || (lMesh>m_md3File.md3Header.lNumMeshes) )
		return E_FAIL;

	//Make sure the frames are within the appropriate range.
	if((lFirstFrame < 0) || (lFirstFrame>=m_md3File.md3Header.lNumFrames))
		return S_FALSE;
	if((lNextFrame < 0) || (lNextFrame>=m_md3File.md3Header.lNumFrames))
		return S_FALSE;
	...
	lNumVertices=m_md3File.md3Mesh[lMesh-1].md3MeshHeader.lNumVertices;
	for(i=0; i<lNumVertices; i++){
		fFirstX=m_md3File.md3Mesh[lMesh-1].md3Vertex[i+lFirstFrame*lNumVertices].x*MD3_XYZ_SCALE;
		fFirstY=m_md3File.md3Mesh[lMesh-1].md3Vertex[i+lFirstFrame*lNumVertices].y*MD3_XYZ_SCALE;
		fFirstZ=m_md3File.md3Mesh[lMesh-1].md3Vertex[i+lFirstFrame*lNumVertices].z*MD3_XYZ_SCALE;

		fNextX=m_md3File.md3Mesh[lMesh-1].md3Vertex[i+lNextFrame*lNumVertices].x*MD3_XYZ_SCALE;
		fNextY=m_md3File.md3Mesh[lMesh-1].md3Vertex[i+lNextFrame*lNumVertices].y*MD3_XYZ_SCALE;
		fNextZ=m_md3File.md3Mesh[lMesh-1].md3Vertex[i+lNextFrame*lNumVertices].z*MD3_XYZ_SCALE;
		...
		m_lpVertices[i].tu=m_md3File.md3Mesh[lMesh-1].md3TexCoords[i].tu;
		m_lpVertices[i].tv=m_md3File.md3Mesh[lMesh-1].md3TexCoords[i].tv;
	}
	...
	IDirect3DDevice9_DrawIndexedPrimitive(
		...
		0,
		0,
		m_md3File.md3Mesh[lMesh-1].md3MeshHeader.lNumVertices,
		0,
		m_md3File.md3Mesh[lMesh-1].md3MeshHeader.lNumTriangles);

	//#define DRAW_NORMAL_MESH
	#ifdef DRAW_NORMAL_MESH
	FLOAT Line[6];
	m_lpDevice->SetTexture(0, NULL);
	for(int k=0; k<m_md3File.md3Mesh[lMesh-1].md3MeshHeader.lNumVertices; k++)
	{
	...
	DWORD dwFlags)
{
	...
	for(i=0; i<m_md3File.md3Header.lNumMeshes; i++){
	...
	#ifdef TESTBOX
	float box[2][3];
	box[0][0]=m_md3File.md3Frame[lFirstFrame].vMin.x;
	box[0][1]=m_md3File.md3Frame[lFirstFrame].vMin.y;
	box[0][2]=m_md3File.md3Frame[lFirstFrame].vMax.z;
	box[1][0]=m_md3File.md3Frame[lFirstFrame].vMax.x;
	box[1][1]=m_md3File.md3Frame[lFirstFrame].vMax.y;
	box[1][2]=m_md3File.md3Frame[lFirstFrame].vMax.z;
	...
	LONG * lpNumMeshes)
{
	...
	*lpNumMeshes=m_md3File.md3Header.lNumMeshes;
	return S_OK;
}
	...
	LONG * lpShaderNum)
{
	...
	if( (lShader <1) || (lShader > m_md3File.md3Mesh[lMesh-1].md3MeshHeader.lNumShaders) )
		return E_FAIL;

	if(lpShaderNum)
		*lpShaderNum=m_md3File.md3Mesh[lMesh-1].md3Shader[lShader-1].lShaderNum;

	strcpy(szShaderName, m_md3File.md3Mesh[lMesh-1].md3Shader[lShader-1].szShaderName);
	return S_OK;
}
	...
	CMD3SkinFile * lpSkin)
{
	...
	for(i=0; i<m_md3File.md3Header.lNumMeshes; i++){
		lpSkin->SetSkinRef(m_md3File.md3Mesh[i].md3MeshHeader.szMeshName, i);
	}
	return S_OK;
}
...
HRESULT CMD3Mesh::CreateVB()
{
	...
	for(i=0; i<m_md3File.md3Header.lNumMeshes; i++){
		dwVerticeSize=m_md3File.md3Mesh[i].md3MeshHeader.lNumVertices * sizeof(D3DMD3VERTEX);
...
HRESULT CMD3Mesh::CreateIB()
{
	...
	for(i=0; i<m_md3File.md3Header.lNumMeshes; i++){
		...
		dwIndexSize=m_md3File.md3Mesh[i].md3MeshHeader.lNumTriangles * 3 * sizeof(SHORT);
		...
		for(j=0, k=0; j<m_md3File.md3Mesh[i].md3MeshHeader.lNumTriangles; j++, k+=3){
			pIndices[k]=(SHORT)m_md3File.md3Mesh[i].md3Triangle[j].nIndexes[0];
			pIndices[k+1]=(SHORT)m_md3File.md3Mesh[i].md3Triangle[j].nIndexes[1];
			pIndices[k+2]=(SHORT)m_md3File.md3Mesh[i].md3Triangle[j].nIndexes[2];
		}
...
HRESULT CMD3Mesh::CreateNormals()
{
	...
	//Get the normals.
	m_lppNormals=(MD3VECTOR**)malloc(sizeof(MD3VECTOR*) * m_md3File.md3Header.lNumMeshes);
	...
	for(i=0; i<m_md3File.md3Header.lNumMeshes; i++)
	{
		m_lppNormals[i]=(MD3VECTOR*)malloc(sizeof(MD3VECTOR) * m_md3File.md3Mesh[i].md3MeshHeader.lNumVertices * m_md3File.md3Mesh[i].md3MeshHeader.lNumFrames);
		...
		for(j=0; j<(m_md3File.md3Mesh[i].md3MeshHeader.lNumVertices*m_md3File.md3Mesh[i].md3MeshHeader.lNumFrames); j++)
		{
			DecodeNormalVector(&m_lppNormals[i][j], &m_md3File.md3Mesh[i].md3Vertex[j]);
		}
	}
	return S_OK;
}
...
HRESULT CMD3Mesh::CreateModel()
{
	...
	m_lppVB=(LPDIRECT3DVERTEXBUFFER9*)malloc( sizeof(LPDIRECT3DVERTEXBUFFER9) * (m_md3File.md3Header.lNumMeshes) );
	if(m_lppVB == NULL){
		return E_FAIL;
	}

	m_lppIB=(LPDIRECT3DINDEXBUFFER9*)malloc( sizeof(LPDIRECT3DINDEXBUFFER9) * (m_md3File.md3Header.lNumMeshes) );
	if(m_lppIB == NULL){
		SAFE_FREE(m_lppVB);
		return E_FAIL;
	}
	...
	for(i=0; i<m_md3File.md3Header.lNumMeshes; i++){
		dwSizeVB = (m_md3File.md3Mesh[i].md3MeshHeader.lNumVertices > (LONG)dwSizeVB) ? m_md3File.md3Mesh[i].md3MeshHeader.lNumVertices : dwSizeVB;
	}
