The files in this folder may not be licensed with this software release. The licenses are as follows:

## laracroft

License: pk3/md3-laracroft.txt

Download: https://ws.q3df.org/model/laracroft/skin/lara/

## Minigun

License: pk3/wep2-M134-Minigun-REAMDME.txt

Download: https://www.lonebullet.com/file/models/q3wep-m134-minigun/2142

## bg.bmp

(C) 2004 Blaine Myers. All rights reserved.

Licensed only for use as demo data with D3DDemo. Other than being distributed with D3DDemo (either the source code for D3DDemo or as a binary executable) distribution of this image is prohibited.

## TestImage2.bmp

Author: Beem Media

License: Same as this software.

## Texture.bmp

Author: Beem Media

License: Same as this software.
